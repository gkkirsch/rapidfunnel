...
(function() {
  ...
  function handleRedirect(url, target) {
    if (url) {
      console.log(`Redirecting to: ${url} ${target === '_blank' ? '(new tab)' : '(same tab)'}`);
      if (target === '_blank') {
  ...
  function initializeCtaTracking(params) {
    ...
    async function sendCtaNotification(payload) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`CTA notification failed: ${response.status} ${response.statusText}`, errorBody);
            } else {
    ...
    async function getContactDetails(contactId) {
        ...
        try {
            const response = await fetch(apiUrl);
            if (!response.ok) {
                console.error(`Failed to fetch contact details: ${response.status} ${response.statusText}`);
                return null;
            }
    ...
    function generateCtaDescription(element) {
        ...
        if (!description) {
            description = element.tagName === 'A' ? 'Unnamed Link' : 'Unnamed Button';
        }
    ...
    console.log(`Found ${trackableElements.length} potential trackable elements (links/buttons). Initializing tracking...`);
    ...
    trackableElements.forEach(element => {
      element.addEventListener('click', async (event) => {
        ...
        console.log(`Click detected on ${element.tagName}: "${ctaButtonLocation}"`);
        
        // --- Prevent tracking form submit buttons as CTAs ---
        if (element.tagName === 'BUTTON') {
            const parentForm = element.closest('form');
            // Check if it's inside a form AND is type="submit" or default type
            if (parentForm && (element.type === 'submit' || !element.hasAttribute('type'))) {
                console.log('Ignoring click on form submit button for CTA tracking.');
        ...
        if (element.tagName === 'A') {
            ...
            if (params.contactId) {
              ...
              console.log('[CTA Link] After await getContactDetails. Details:', contactDetails);
              if (contactDetails) {
                notificationPayload.contactFirstName = contactDetails.firstName || "N/A";
                notificationPayload.contactLastName = contactDetails.lastName || "N/A";
                notificationPayload.contactPhoneNumber = contactDetails.phone || "N/A";
                notificationPayload.contactEmail = contactDetails.email || "N/A";
              } else {
                notificationPayload.contactFirstName = "System failed to retrieve";
            ...
            try {
                ...
                console.error("[CTA Link] Notification sending failed (handled).", error);
            } finally {
                console.log('[CTA Link] Inside finally block, before handleRedirect.');
                handleRedirect(redirectUrl, target);
                console.log('[CTA Link] Inside finally block, after handleRedirect call.');
            }
            ...
            sendCtaNotification(notificationPayload) // Send async, don't wait
              .then(() => {
                ...
                if (redirectUrl) {
                    console.log('Handling custom redirect for button:', redirectUrl);
  ...
  function initializeVideoTracking(params) {
    ...
    async function sendVideoPlayData(payload) {
      ...
      try {
        ...
        if (!response.ok) {
            ...
            console.error(`Video tracking submission failed: ${response.status} ${response.statusText}`, errorBody);
            // The original docs showed a bootbox confirm, we'll just log for now.
            // alert('Error tracking video play. Please reload.');
        } else {
            ...
            if (contentType && contentType.indexOf("application/json") !== -1) {
                ...
                if (!responseData) {
                 ...
                 console.log('Video tracking submitted successfully (non-JSON response).');
            }
        }
      } catch (error) {
    ...
    if (typeof window._wq === 'undefined') {
      console.log('Wistia Player API (_wq) not found. Skipping video tracking initialization.');
      return;
    }
    ...
    window._wq.push({
      _all: function(video) {
        ...
        video.bind('play', () => {
          ...
          if (!isValidNumber(userId) || !isValidNumber(resourceId) || !isValidNumber(contactId)) {
            console.warn('Cannot track video play: Missing or non-numeric userId, resourceId, or contactId in URL params.');
            return;
          }
    ...
    console.log('Wistia video tracking initialized. Waiting for play events...');
  }
  ...
  function initializeFormTracking(params) {
    ...
    function findFormFields(form) {
        ...
        for (const fieldKey in patterns) {
            ...
            if (fields[inputKey] && fieldKey !== 'email' && fieldKey !== 'phone') continue; 
            ...
            patterns[fieldKey].some(regex => { 
                return Array.from(form.elements).some(input => {
                    ...
                    if (!['text', 'email', 'tel', 'hidden'].includes(input.type.toLowerCase())) return false;
                    ...
                    return attrs.some(attr => {
                        ...
                        if (value && regex.test(value)) {
                            ...
                            if ((fieldKey === 'email' && fields.emailInput === input) || 
        ...
        if (!canTrack) {
            console.log('Form cannot be tracked (missing email and phone): ', form.id || 'Unnamed');
            return null; // Indicate cannot track this form
        }

        console.log('Identified potential contact fields in form:', form.id || 'Unnamed', fields);
    ...
    async function submitContactForm(formDataString, resourceId, senderId, formElement) {
      const apiUrl = 'https://my.rapidfunnel.com/landing/resource/create-custom-contact'; 
      ...
      try {
          ...
          if (!response.ok) {
              let errorBody = 'Could not read error response.';
              try { errorBody = await response.json(); } catch (e) { /* Ignore */ }
              console.error(`Contact form submission failed: ${response.status} ${response.statusText}`, errorBody);
              return null; 
          }
          const responseData = await response.json();
          ...
          if (!newContactId || newContactId <= 0) {
              console.error('Form submitted but no valid contactId received.', responseData);
              return null;
          }
          let nextPageUrl = window.rapidFunnelNextPage; 
          if (!nextPageUrl) {
              const submitContainer = document.getElementById('contactFormSubmitContainer'); 
              if (submitContainer) { nextPageUrl = submitContainer.getAttribute('data-redirect'); }
          }
          if (nextPageUrl) {
    ...
    console.log(`Found ${allForms.length} form(s). Attempting to identify trackable contact forms...`);
    ...
    allForms.forEach(form => {
      ...
      if (identifiedFields) {
        trackedFormsCount++;
        console.log(`Attaching submit listener to form (found email/phone):`, form.id || 'Unnamed form');
        ...
        form.addEventListener('submit', async (event) => {
          ...
          const { firstNameInput, lastNameInput, nameInput, emailInput, phoneInput } = identifiedFields;
          ...
          if (!emailValue && !phoneValue) {
              ...
              console.warn('Form submission stopped: Missing both email and phone value.');
              return;
          }
          ...
          } else if (firstNameInput?.value) { // Only first name found
              firstName = firstNameInput.value.trim();
          } else if (lastNameInput?.value) { // Only last name found (use as first?)
          ...
          if (!params.userId || !params.resourceId) {
            console.error('Cannot submit form: Missing userId or resourceId in URL.');
            alert('Cannot submit form: Required tracking information is missing.');
            return; 
          }
          ...
          const newContactId = await submitContactForm(formDataString, resourceId, senderId, form);

          // Re-enable button logic remains the same
           if (submitButton && (!newContactId || !window.rapidFunnelNextPage && !document.getElementById('contactFormSubmitContainer')?.getAttribute('data-redirect'))) { 
    ...
    if (trackedFormsCount === 0) {
        console.log('Could not automatically identify any forms with at least an email or phone field.');
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!params.userId || !params.resourceId) {
      console.warn('RapidFunnel Tracker: userId or resourceId missing from URL parameters. Tracking might be limited.');
